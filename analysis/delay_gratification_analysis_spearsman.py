...
from utils import get_spearman_distance
...
#             else:
#                 max_res = max(max_res, abs(n))
#                 leading_zero = 0
#     return max_res

def avg_wandering_range_2(arr):
    d_arr = get_discrepency_arr(arr)
    leading_zero = 0
    res = []
    for n in d_arr:
...
def get_max_delay_gratification(steps):
    monotonicity_changes = [get_spearman_distance(step) for step in steps if len(step) > 0]
    ...
    return avg_wandering_range_2(monotonicity_changes)
